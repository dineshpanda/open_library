...
RSpec.describe "Books", type: :request do
  ...
  describe "GET /index" do
    it "returns http success" do
      ...
      expect(response.parsed_body).to include(book.as_json(only: %i[title author published_on]))
    end
  end

  describe "POST /checkout" do
    it "checkouts available book successfully" do
      post book_checkout_path(book), params: { user_id: user.id }
      expect(response).to have_http_status(:success)
    ...
    it "returns 404 if required params are missing" do
      post book_checkout_path(book)
      expect(response).to have_http_status(:not_found)
    end

    it "returns 404 if invalid params are passed" do
      post book_checkout_path(book), params: { user_id: 0 }
      expect(response).to have_http_status(:not_found)
    end
